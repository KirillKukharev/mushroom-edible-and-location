...
function add_marker(map, markers, markersVisibility, heatmap_points, data) {
    ...
    if (!!data.location_name && data.location_name.includes('В этом месте могут быть грибы с вероятностью'))
        var marker = new google.maps.Marker({
          position: latlon,
          map: map,
          visible: markersVisibility.valueOf(),
          icon:'/static/images/mush_find.png',
          title: 'Mushroom'
        });
        ...
        var marker = new google.maps.Marker({
          ...
          map: map,
          visible: markersVisibility.valueOf(),
          icon:'/static/images/orange_MarkerO.png',
          title: 'Mushroom'
        });
    else
        var marker = new google.maps.Marker({
          ...
          map: map,
          visible: markersVisibility.valueOf(),
          icon:'/static/images/purple_MarkerO.png',
          title: 'Mushroom'
        });
    marker.addListener('click', function() {
      if (!!data.location_name && data.location_name.includes('В этом месте могут быть грибы с вероятностью'))
          infowindow.setContent('<img src=' + data.url + '>' + '<br>' +'<p>'+ data.location_name +'</p>');
      else if (!!data.location_name)
          infowindow.setContent('<img src=' + data.url + '>' + '<br>' +'<p>'+ data.location_name + '<br>'+ 'Добавлено пользователем: ' + data.username + '</p>');
      else
...
function initialize() {
    ...
    var centerlatlng = new google.maps.LatLng(57.6261, 39.8845);
